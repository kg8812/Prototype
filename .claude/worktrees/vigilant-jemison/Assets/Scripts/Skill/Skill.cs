...
namespace Apis
{
    public abstract partial class Skill : SerializedScriptableObject, IAttackItemStat
    {
        ...
        protected virtual void UpdateEvent(EventParameters parameters)
        {
            ...
            if (CdUse) CDActive.Update(parameters);
        ...
        {
            stats = baseConfig;
            attachments.ForEach(x => { stats = new SkillDecorator(stats, x); });
        }
        ...
        public virtual bool Use()
        {
            ...
            CDActive.SetCD();
            if (_cdStartType == CdStartType.OnUse) CDActive.StartCd();
            return true;
        }

        public virtual void Active()
        {
            if (_cdStartType == CdStartType.AfterCharge) CDActive.StartCd();
            ...
            if (isMotion)
            {
                if (EnterSkillState && user is Player player) player.SetState(EPlayerState.Skill);
        ...
        public virtual IEnumerator DurationCoroutine()
        {
            ...
            while (curDuration > 0)
            {
                if (!isStop) curDuration -= Time.deltaTime;
        ...
        protected virtual void StopDuration()
        {
            if (durationCoroutine != null)
            {
                ...
                curDuration = 0;
                AfterDuration();
                if (_cdStartType == CdStartType.AfterDuration) CDActive.StartCd();
        ...
        {
            if (!unEquipCd) eventUser?.EventManager.RemoveEvent(EventType.OnUpdate, UpdateEvent);
        ...
        {
            StopDuration();
            if (_cdStartType == CdStartType.OnCancel) CDActive.StartCd();
            RemoveCancelEvents();
        }
        ...
        private void RemoveCancelEvents()
        {
        ...
        private void CancelInvoke(EventParameters parameters)
        {
            Cancel();
        }
        ...
        [TitleGroup("사용 관련")] [TabGroup("사용 관련/설정", "기본설정")] [LabelText("쿨타임 적용 시기")] [SerializeField]
        protected CdStartType _cdStartType = CdStartType.OnUse;

        [TabGroup("사용 관련/설정", "기본설정")] [LabelText("쿨타임")] [SerializeField]
        private float baseCd;

        [TabGroup("사용 관련/설정", "기본설정")] [LabelText("지속시간")] [SerializeField]
        private float duration;

        [TabGroup("사용 관련/설정", "기본설정")] [LabelText("모션여부")] [SerializeField]
        ...
        [LabelText("장착해제시 쿨타임 활성화 여부")] [SerializeField] [Tooltip("체크해제 : 장착해제시 쿨타임 동결")] [TabGroup("사용 관련/설정", "기본설정")]
        private bool unEquipCd;

        [FormerlySerializedAs("EnterChargeState")] [TabGroup("사용 관련/설정", "기본설정")] [LabelText("스킬상태 들어가는 여부")]
        public bool EnterSkillState = true;
        ...
        [TitleGroup("스탯값")] [LabelText("신체 피해%")] [SerializeField]
        private float bodyFactor;

        [TitleGroup("스탯값")] [LabelText("영혼 피해%")] [SerializeField]
        private float spiritFactor;

        [TitleGroup("스탯값")] [LabelText("기량 피해%")] [SerializeField]
        private float finesseFactor;
        ...
        [FormerlySerializedAs("")] [Title("string 인덱스")] [LabelText("이름")] [SerializeField]
        protected int nameString;
        ...
        public float Duration
        {
            get
            {
                if (stats?.Stat != null) return (duration + stats.Stat.duration) * (1 + stats.Stat.durationRatio / 100);
        ...
        public int StackGain
        {
            get
            {
                if (stats?.Stat != null) return stackGain + stats.Stat.stackGain;
        ...
        public virtual float Atk
        {
            get
            {
                if (stats?.Stat != null) return (baseDmg + stats.Stat.dmg) * (1 + stats.Stat.dmgRatio / 100);
                return baseDmg;
            }
        }
        ...
        public UnityEvent<int> OnStackChange => _onStackChange ??= new UnityEvent<int>();
        ...
        protected List<ISkill> attachments => _attachments ??= new List<ISkill>();
        ...
        public UnityEvent OnAfterDuration => _onAfterDuration ??= new UnityEvent();
        ...
        public virtual float CurCd
        {
            ...
            set
            {
                if (CDActive != null) CDActive.CurCd = value;
            }
        }
        ...
        public UnityEvent OnActive => _onActive ??= new UnityEvent();


        private UnityEvent _onUpdate = new();
        public UnityEvent OnUpdate => _onUpdate ??= new UnityEvent();
