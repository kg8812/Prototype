...
public class ObjectPool<T> where T : Component
{
    ...
    private readonly GameObject pool; // 생성된 오브젝트들 정리용, 정리 외 용도 X

    // 생성용 프리팹 Dictionary, Key 값으론 프리팹 이름 들어감
    private readonly IDictionary<string, T> prefabs = new Dictionary<string, T>();

    // 오브젝트들 관리용 Queue Dictionary, Key 값으로 동일하게 프리팹 이름 들어감
    private readonly IDictionary<string, Queue<T>> queue = new Dictionary<string, Queue<T>>();
    ...
    private void CreateNew(string name) // 내부용 오브젝트 생성 함수, 프리팹 이름 매개변수로 기입
    {
        ...
        {
            var obj = Object.Instantiate(prefabs[name]).GetOrAddComponent<T>();
            obj.name = name;
    ...
    public T Get(string name) // 외부에서 오브젝트 가져올 때 사용, 매개변수로 프리팹 이름 기입
    {
        ...
        if (queue.ContainsKey(name))
        {
            ...
            if (queue[name].Count == 0) CreateNew(name);
