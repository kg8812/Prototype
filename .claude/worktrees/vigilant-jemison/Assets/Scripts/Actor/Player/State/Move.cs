    ...
    public class Move : BaseGroundState, IAnimate
    {
        ...
        public override void FixedUpdate()
        {
            ...
            if (_player.ActorMovement.CheckMovable())
                _player.MoveComponent.ForceActorMovement.Move(_player.Direction, 1, false, _player.MoveResistFactor,
                    _player.MaxMoveVel);
            else
                _player.StopMoving();

            var _moveMultiplier = 1 * _player.MoveSpeed / _player.StatManager.BaseStat.Get(ActorStatType.MoveSpeed);
            var normalizedSpeed = _player.Rb.linearVelocity.magnitude / _player.MaxMoveVel *
                                  Mathf.Sign(_player.Rb.linearVelocity.x);
