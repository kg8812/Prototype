...
public abstract class ES3Writer : IDisposable
{
    ...
    protected void Merge(ES3Reader reader)
    {
        foreach (KeyValuePair<string, ES3Data> kvp in reader.RawEnumerator)
            if (!keysToDelete.Contains(kvp.Key) ||
                kvp.Value.type == null) // Don't add keys whose data is of a type which no longer exists in the project.
    ...
    /// <param name="value">The value we want to write.</param>
    /// <param name="memberReferenceMode">
    ///     Whether we want to write UnityEngine.Object fields and properties by reference, by
    ///     value, or both.
    /// </param>
    [EditorBrowsable(EditorBrowsableState.Never)]
    ...
    [EditorBrowsable(EditorBrowsableState.Never)]
    public virtual void Write(object value, ES3Type type,
        ES3.ReferenceMode memberReferenceMode = ES3.ReferenceMode.ByRef)
    {
        // Note that we have to check UnityEngine.Object types for null by casting it first, otherwise
        // it will always return false.
        if (value == null ||
            (ES3Reflection.IsAssignableFrom(typeof(Object), value.GetType()) && value as Object == null))
        {
            WriteNull();
            return;
        }
        ...
        if (type == null || type.type == typeof(object))
        {
            ...
            if (type == null)
                throw new NotSupportedException("Types of " + valueType +
                                                " are not supported. Please see the Supported Types guide for more information: https://docs.moodkie.com/easy-save-3/es3-supported-types/");
        ...
        if (type.isUnsupported)
        {
            if (type.isCollection || type.isDictionary)
                throw new NotSupportedException(type.type +
                                                " is not supported because it's element type is not supported. Please see the Supported Types guide for more information: https://docs.moodkie.com/easy-save-3/es3-supported-types/");
            throw new NotSupportedException("Types of " + type.type +
                                            " are not supported. Please see the Supported Types guide for more information: https://docs.moodkie.com/easy-save-3/es3-supported-types/");
        }
    ...
    internal virtual void WriteRef(Object obj, string propertyName)
    {
        WriteRef(obj, ES3ReferenceMgrBase.referencePropertyName, ES3ReferenceMgrBase.Current);
    }

    internal virtual void WriteRef(Object obj, string propertyName, ES3ReferenceMgrBase refMgr)
    {
        if (refMgr == null)
    ...
    public void WritePrivateProperty(string name, object objectContainingProperty)
    {
        ...
        WriteProperty(name, property.GetValue(objectContainingProperty),
            ES3TypeMgr.GetOrCreateES3Type(property.MemberType));
    }
    ...
    public void WritePrivatePropertyByRef(string name, object objectContainingProperty)
    {
        ...
        if (property.IsNull)
            throw new MissingMemberException("A private property named " + name + " does not exist in the type " +
                                             objectContainingProperty.GetType());
        WritePropertyByRef(name, (Object)property.GetValue(objectContainingProperty));
    }
    ...
    public void WritePrivateFieldByRef(string name, object objectContainingField)
    {
        ...
        if (field.IsNull)
            throw new MissingMemberException("A private field named " + name + " does not exist in the type " +
                                             objectContainingField.GetType());
        WritePropertyByRef(name, (Object)field.GetValue(objectContainingField));
    }
