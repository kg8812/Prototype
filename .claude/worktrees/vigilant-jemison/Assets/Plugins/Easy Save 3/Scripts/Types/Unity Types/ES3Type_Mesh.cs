...
{
    [Preserve]
    [ES3PropertiesAttribute("bounds", "subMeshCount", "boneWeights", "bindposes", "vertices", "normals", "tangents",
        "uv", "uv2", "uv3", "uv4", "colors32", "triangles", "subMeshes")]
    public class ES3Type_Mesh : ES3UnityObjectType
    {
        ...
        protected override void WriteUnityObject(object obj, ES3Writer writer)
        {
            ...
            {
                ES3Debug.LogWarning(
                    "Easy Save cannot save the vertices for this Mesh because it is not marked as readable, so it will be stored by reference. To save the vertex data for this Mesh, check the 'Read/Write Enabled' checkbox in its Import Settings.",
                    instance);
                return;
            }
            ...
            for (var i = 0; i < instance.subMeshCount; i++)
            ...
            for (var blendShapeIndex = 0; blendShapeIndex < instance.blendShapeCount; blendShapeIndex++)
            {
                writer.WriteProperty("GetBlendShapeName" + blendShapeIndex,
                    instance.GetBlendShapeName(blendShapeIndex));
                writer.WriteProperty("GetBlendShapeFrameCount" + blendShapeIndex,
                    instance.GetBlendShapeFrameCount(blendShapeIndex));

                for (var frameIndex = 0; frameIndex < instance.GetBlendShapeFrameCount(blendShapeIndex); frameIndex++)
                {
                    ...
                    instance.GetBlendShapeFrameVertices(blendShapeIndex, frameIndex, deltaVertices, deltaNormals,
                        deltaTangents);
                    ...
                    writer.WriteProperty("blendShapeFrameWeight" + blendShapeIndex + "_" + frameIndex,
                        instance.GetBlendShapeFrameWeight(blendShapeIndex, frameIndex));
                }
            }
        }
        ...
        protected override void ReadUnityObject<T>(ES3Reader reader, object obj)
        {
            ...
            if (!instance.isReadable)
                ES3Debug.LogWarning(
                    "Easy Save cannot load the vertices for this Mesh because it is not marked as readable, so it will be loaded by reference. To load the vertex data for this Mesh, check the 'Read/Write Enabled' checkbox in its Import Settings.",
                    instance);
            ...
            foreach (string propertyName in reader.Properties)
            {
                ...
                {
                    case "indexFormat":
                        instance.indexFormat = reader.Read<IndexFormat>();
                        break;
                    case "name":
                    ...
                    case "colors32":
                        ...
                        break;
                    case "triangles":
                        instance.triangles = reader.Read<int[]>(ES3Type_intArray.Instance);
                        break;
                    case "subMeshCount":
                        instance.subMeshCount = reader.Read<int>(ES3Type_int.Instance);
                        for (var i = 0; i < instance.subMeshCount; i++)
                            instance.SetTriangles(reader.ReadProperty<int[]>(ES3Type_intArray.Instance), i);
                        break;
                    case "blendShapeCount":
                        instance.ClearBlendShapes();
                        var blendShapeCount = reader.Read<int>(ES3Type_int.Instance);
                        for (var blendShapeIndex = 0; blendShapeIndex < blendShapeCount; blendShapeIndex++)
                        {
                            ...
                            for (var frameIndex = 0; frameIndex < frameCount; frameIndex++)
                            {
                                ...
                                instance.AddBlendShapeFrame(shapeName, frameWeight, deltaVertices, deltaNormals,
                                    deltaTangents);
                            }
                        }
