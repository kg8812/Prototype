...
public abstract class ES3Reader : IDisposable
{
    ...
    public virtual ES3ReaderPropertyEnumerator Properties => new(this);

    internal virtual ES3ReaderRawEnumerator RawEnumerator => new(this);
    ...
    /// <summary>
    ///     Reads a property (i.e. a property name and value) from the reader, ignoring the property name and only
    ///     returning the value.
    /// </summary>
    [EditorBrowsable(EditorBrowsableState.Never)]
    ...
    /// <returns>
    ///     The objectContainingProperty object. This is helpful if you're setting a private property on a struct or other
    ///     immutable type and need to return the boxed value.
    /// </returns>
    public object SetPrivateProperty(string name, object value, object objectContainingProperty)
    {
        ...
        if (property.IsNull)
            throw new MissingMemberException("A private property named " + name + " does not exist in the type " +
                                             objectContainingProperty.GetType());
    ...
    /// <returns>
    ///     The objectContainingField object. This is helpful if you're setting a private property on a struct or other
    ///     immutable type and need to return the boxed value.
    /// </returns>
    public object SetPrivateField(string name, object value, object objectContainingField)
    {
        ...
        if (field.IsNull)
            throw new MissingMemberException("A private field named " + name + " does not exist in the type " +
                                             objectContainingField.GetType());
    ...
    internal Type ReadTypeFromHeader<T>()
    {
        ...
        if (settings.typeChecking)
        {
            ...
            if (type == null)
                throw new TypeLoadException("Trying to load data of type " + typeof(T) +
                                            ", but the type of data contained in file no longer exists. This may be because the type has been removed from your project or renamed.");
            if (type != typeof(T) && !ES3Reflection.IsAssignableFrom(typeof(T), type))
                throw new InvalidOperationException("Trying to load data of type " + typeof(T) +
                                                    ", but data contained in file is type of " + type + ".");
    ...
    {
        var stream = ES3Stream.CreateStream(settings, ES3FileMode.Read);
        if (stream == null)
            return null;
    ...
    {
        var stream = ES3Stream.CreateStream(new MemoryStream(bytes), settings, ES3FileMode.Read);
        if (stream == null)
            return null;
    ...
    public class ES3ReaderPropertyEnumerator
    {
        ...
        public IEnumerator GetEnumerator()
        {
            ...
            while (true)
                ...
                {
                    var tempName = reader.overridePropertiesName;
    ...
    public class ES3ReaderRawEnumerator
    {
        ...
        public IEnumerator GetEnumerator()
        {
            while (true)
            {
                ...
                var type = reader.ReadTypeFromHeader<object>();

                var bytes = reader.ReadElement();
    ...
    public virtual T Read<T>(string key)
    {
        ...
        var obj = Read<T>(ES3TypeMgr.GetOrCreateES3Type(type));
    ...
    public virtual T Read<T>(ES3Type type)
    {
        if (type == null || type.isUnsupported)
            throw new NotSupportedException("Type of " + type +
                                            " is not currently supported, and could not be loaded using reflection.");
        if (type.isPrimitive)
    ...
    {
        if (type == null || type.isUnsupported)
            throw new NotSupportedException("Type of " + obj.GetType() +
                                            " is not currently supported, and could not be loaded using reflection.");
