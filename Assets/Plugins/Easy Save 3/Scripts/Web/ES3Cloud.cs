...
public class ES3Cloud : ES3WebClass
{
    ...
    /// <summary>
    ///     Any downloaded data, if applicable. This may also contain an error message, so you should check the 'ifError'
    ///     variable before reading data.
    /// </summary>
    ...
    /// <summary>
    ///     An array of filenames downloaded from the server. This must only be accessed after calling the
    ///     'DownloadFilenames' routine.
    /// </summary>
    ...
    /// <summary>
    ///     A UTC DateTime object representing the date and time a file on the server was last updated. This should only
    ///     be called after calling the 'DownloadTimestamp' routine.
    /// </summary>
    public DateTime timestamp
    {
        get
        {
            ...
            if (!double.TryParse(text, out timestamp))
                throw new FormatException("Could not convert downloaded data to a timestamp. Data downloaded was: " +
                                          text);
    ...
    /// <summary>
    ///     Synchronises the default file with a file on the server. If the file on the server is newer than the local
    ///     copy, the local file will be overwritten by the file on the server. Otherwise, the file on the server will be
    ///     overwritten.
    /// </summary>
    ...
    /// <summary>
    ///     Synchronises a local file with a file on the server. If the file on the server is newer than the local copy,
    ///     the local file will be overwritten by the file on the server. Otherwise, the file on the server will be
    ///     overwritten.
    /// </summary>
    ...
    /// <summary>
    ///     Downloads the default file from the server and saves it locally, overwriting the existing local default file.
    ...
    /// <summary>
    ///     Downloads a file from the server and saves it locally, overwriting any existing local file. An error is
    ...
    /// </summary>
    /// <param name="es3File">
    ///     The ES3File we want to load our data into. The filename in the settings of the ES3File will be
    ...
    /// <summary>
    ///     Downloads the names of all of the files on the server. Downloaded filenames are stored in the 'filenames'
    ///     variable of the ES3Cloud object.
    /// </summary>
    ...
    ///     A search pattern containing '%' or '_' wildcards where '%' represents zero, one, or
    ...
    /// <summary>
    ///     Downloads the timestamp representing when the server file was last updated. The downloaded timestamp is stored
    ///     in the 'timestamp' variable of the ES3Cloud object.
    /// </summary>
    ...
    {
        return Convert.ToInt64((dt.ToUniversalTime() - new DateTime(1970, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc))
            .TotalSeconds);
    }
