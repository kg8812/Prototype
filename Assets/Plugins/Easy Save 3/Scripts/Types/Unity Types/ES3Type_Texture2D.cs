...
namespace ES3Types
{
    ...
    public class ES3Type_Texture2D : ES3UnityObjectType
    {
        ...
        protected override void ReadUnityObject<T>(ES3Reader reader, object obj)
        {
            ...
            if (!IsReadable(instance))
                ES3Debug.LogWarning(
                    $"Easy Save cannot load the properties or pixels for this Texture {instance.name} because it is not read/write enabled, so it will be loaded by reference. To load the properties and pixels for this Texture, check the 'Read/Write Enabled' checkbox in its Import Settings.",
                    instance);
            ...
            foreach (string propertyName in reader.Properties)
            {
                ...
                switch (propertyName)
                {
                    ...
                    case "rawTextureData":
                        ...
                        {
                            ES3Debug.LogWarning(
                                "Easy Save cannot load the pixels of this Texture because it is not read/write enabled, so Easy Save will ignore the pixel data. To load the pixel data, check the 'Read/Write Enabled' checkbox in the Texture's import settings. Clicking this warning will take you to the Texture, assuming it is not generated at runtime.",
                                instance);
                            reader.Skip();
                        }
                        else
                        {
                            ...
                            {
                                ES3Debug.LogError(
                                    "Easy Save encountered an error when trying to load this Texture, please see the end of this messasge for the error. This is most likely because the Texture format of the instance we are loading into is different to the Texture we saved.\n" +
                                    e, instance);
                            }
                        }
        ...
        {
            var instance = new Texture2D(
                reader.Read<int>(ES3Type_int
                    .Instance), // Property name has already been read in ES3UnityObjectType, so we only need to read the value.
