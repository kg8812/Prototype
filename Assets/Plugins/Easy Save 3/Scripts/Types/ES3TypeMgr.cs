...
namespace ES3Internal
{
    ...
    public static class ES3TypeMgr
    {
        ...
        internal static ES3Type CreateES3Type(Type type, bool throwException = true)
        {
            ...
            {
                var genericType = ES3Reflection.GetGenericTypeDefinition(type);
                ...
                else if (genericType == typeof(NativeArray<>))
            ...
            {
                var rank = ES3Reflection.GetArrayRank(type);
                if (rank == 1)
                    ...
                    throw new NotSupportedException(
                        "Only arrays with up to three dimensions are supported by Easy Save.");
                else
                    return null;
            }
            ...
            {
                if (types == null ||
                    types.Count == 0) // If the type list is not initialised, it is most likely an initialisation error.
                    throw new TypeLoadException(
                        "ES3Type for primitive could not be found, and the type list is empty. Please contact Easy Save developers at https://www.moodkie.com/contact");
                // Else it's a different error, possibly an error in the specific ES3Type for that type.
                throw new TypeLoadException(
                    "ES3Type for primitive could not be found, but the type list has been initialised and is not empty. Please contact Easy Save developers using the form at https://www.moodkie.com/contact.");
            }
            else
            {
                ...
                else if (ES3Reflection.IsAssignableFrom(typeof(Object), type))
                {
            ...
            if (es3Type == null || es3Type.type == null || es3Type.isUnsupported)
            {
                if (throwException)
                    throw new NotSupportedException(
                        $"ES3Type.type is null when trying to create an ES3Type for {type}, possibly because the element type is not supported or there is no generic ES3Type for this class.");
                return null;
            }
        ...
        private static ES3Type GetGenericES3Type(Type type, bool throwException)
        {
            ...
            if (es3Type != null)
            {
                ...
                var constructor = ES3Reflection.GetConstructor(es3Type.GetType(), new[] { typeof(Type) });

                if (constructor == null)
                {
                    if (throwException)
                        throw new NotSupportedException(
                            $"Generic type {type} is not supported by Easy Save as it's generic type definition {genericType} does not have a constructor which accepts a Type as a parameter.");
                    return null;
                }
            ...
            if (throwException)
                throw new NotSupportedException("Generic type \"" + type + "\" is not supported by Easy Save.");
        ...
        internal static void Init()
        {
            lock (_lock)
            {
                ...
                var instances =
                    ES3Reflection
                        .GetInstances<
                            ES3Type>(); // ES3Types add themselves to the manager when instantiated to ensure they don't cause cyclic references if they contain a field which is the same type as themselves.
                ...
                if (types == null || types.Count == 0)
                    throw new TypeLoadException(
                        "Type list could not be initialised. Please contact Easy Save developers on mail@moodkie.com.");
            }
        }
    }
}