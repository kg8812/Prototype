...
namespace ES3Internal
{
    ...
    public class ES3JSONReader : ES3Reader
    {
        ...
        internal ES3JSONReader(Stream stream, ES3Settings settings, bool readHeaderAndFooter = true) : base(settings,
            readHeaderAndFooter)
        {
        ...
        public override string ReadPropertyName()
        {
            ...
            if (c == ',')
                ...
                throw new FormatException("Expected ',' separating properties or '\"' before property name, found '" +
                                          c + "'.");
        ...
        protected override Type ReadKeyPrefix(bool ignoreType = false)
        {
            ...
            if (propertyName != "value")
                throw new FormatException(
                    "This data is not Easy Save Key Value data. Expected property name \"value\", found \"" +
                    propertyName + "\".");
        ...
        internal override bool EndReadDictionaryValue()
        {
            ...
            if (c != ',')
                throw new FormatException(
                    "Expected ',' seperating Dictionary items or '}' terminating Dictionary, found '" + c + "'.");
            return false;
        }
        ...
        internal override bool EndReadCollectionItem()
        {
            ...
            if (c != ',')
                throw new FormatException(
                    "Expected ',' seperating collection items or ']' terminating collection, found '" + c + "'.");
            return false;
        }
        ...
        private void ReadString(StreamWriter writer, bool skip = false)
        {
            ...
            while (!endOfString)
            {
                var c = ReadOrSkipChar(writer, skip);
                switch (c)
                {
        ...
        internal override byte[] ReadElement(bool skip = false)
        {
            ...
            var writer = skip ? null : new StreamWriter(new MemoryStream(settings.bufferSize));

            using (writer)
            {
                var nesting = 0;
                var c = (char)baseReader.Peek();
        ...
        private void SkipOpeningBraceOfFile()
        {
            ...
            if (firstChar != '{') // If first char isn't '{', it's not valid JSON.
                throw new FormatException("File is not valid JSON. Expected '{' at beginning of file, but found '" +
                                          firstChar + "'.");
        }
        ...
        internal override long Read_ref()
        {
            if (ES3ReferenceMgr.Current == null)
                throw new InvalidOperationException(
                    "An Easy Save 3 Manager is required to load references. To add one to your scene, exit playmode and go to Tools > Easy Save 3 > Add Manager to Scene");
