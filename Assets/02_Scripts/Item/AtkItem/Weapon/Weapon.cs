...
namespace Apis
{
    public abstract class Weapon : Item, IAttackItem, IAttackItemStat
    {
        ...
        public virtual void Attack(int count)
        {
            ...
            else if (count > 0) GroundComboList.Invoke(count - 1);
        }
        ...
        public override void Init()
        {
            ...
            if (Image == null) Image = ResourceUtil.Load<Sprite>(_data.iconPath);

            wpSprites = GetComponentsInChildren<WeaponSprite>(true);
            foreach (var x in wpSprites) x.ActiveRenderer(false);
        }
        ...
        {
            if (Skill != null && Skill.isActive) Skill.Use();
        }
        ...
        protected override void OnUnEquip()
        {
            ...
            if (!ReferenceEquals(Skill, null)) Skill.UnEquip();
            IAttack?.OnAfterAtk?.Invoke();
        }
        ...
        private void SetBoneFollwer(bool isOn)
        {
            if (followers != null)
                foreach (var x in followers.Keys)
                {
                    var follower = Utils.GetOrAddComponent<BoneFollower>(x.gameObject);
        ...
        public struct atkInfo
        {
            ...
            [LabelText("그로기 넉백")] public KnockBackData KnockBackData;
        }
        ...
        [FormerlySerializedAs("skill")] [SerializeField]
        private MagicSkill _skill;
        ...
        public virtual Vector2 FirePos
        {
            get
            {
                if (firePos == null) return GameManager.instance.ControllingEntity.Position;
        ...
        [LabelText("지상공격 데미지")]
        public List<atkInfo> groundAtkDmgs = new();

        [TabGroup("공격설정/설정", "공중")] [ShowIf("attackType", AttackType.Collider)] [LabelText("공중공격 데미지")]
        public List<atkInfo> airAtkDmgs = new();

        [LabelText("따라다니기")] [SerializeField] private bool isFollow;

        [InfoBox("BoneFollower 적용하는 오브젝트들 \n 값으로는 boneName 넣을 것")]
        public Dictionary<GameObject, string> followers;
        ...
        public virtual float Atk => _config.BonusStat.Stats[ActorStatType.Atk].Value *
                                    (1 + _config.BonusStat.Stats[ActorStatType.Atk].Ratio / 100f);
        ...
        public BoneFollower[] BoneFollower =>
            _boneFollower ??= followers?.Select(x => x.Key.GetComponent<BoneFollower>()).Where(x => x != null)
                .ToArray();
        ...
        public virtual List<float> GroundCancelTimes =>
            groundCancelTimes ??= groundAtkDmgs.Select(x => x.cancelTime).ToList();
        ...
        public UnityEvent<EventParameters> OnAtkObjectInit => _onAtkObjectInit ??= new UnityEvent<EventParameters>();
