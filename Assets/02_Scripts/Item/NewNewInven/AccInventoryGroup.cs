...
namespace NewNewInvenSpace
{
    public class AccInventoryGroup : InventoryGroup
    {
        ...
        public AccInventoryGroup(int eqMaxCnt, int eqCnt, int stMaxCnt, int stCnt) : base(eqMaxCnt, eqCnt, stMaxCnt,
            stCnt)
        {
        ...
        public BonusStat BonusStat
        {
            get
            {
                ...
                _bonusStat.Reset();
                for (var i = 0; i < Invens[InvenType.Equipment].Count; i++)
                {
                    var item = Invens[InvenType.Equipment][i];
                    if (item != null && item is Accessory acc) _bonusStat += acc.BonusStat;
                }
        ...
        {
            var item = base.Abandon(index, type);

            if (item != null && item is Accessory accessory)
            {
                var pu = GameManager.Item.AccPickUp.CreateExisting(InvenManager.instance.Storage.Get(accessory));
        ...
        {
            if (item != null && item is Accessory acc) acc.Equip(GameManager.instance.Player);
        }

        private void UnEquipped(int index, Item item)
        {
            if (item != null && item is Accessory acc) acc.UnEquip();
        }
    }
}