...
namespace Apis
{
    public class EffectSpawner
    {
        ...
        private List<ParticleDestroyer> allEffects = new();

        public EffectSpawner(Actor user)
        {
            _user = user;
            user?.OnHide.AddListener(() => { TurnEffectsOnOrOff(false); });
            user?.OnAppear.AddListener(() => { TurnEffectsOnOrOff(true); });
            user?.AddEvent(EventType.OnEnable, _ => { TurnEffectsOnOrOff(true); });
            user?.AddEvent(EventType.OnDisable, _ => { TurnEffectsOnOrOff(false); });
        ...
        public ParticleDestroyer Spawn(string address, Vector2 position, bool setEffectParent,
            bool disappearWhenHide = false)
        {
        ...
        public ParticleDestroyer SpawnAndDestroyAfterDuration(string address, string boneName, UnityAction afterDestroy,
            bool setEffectParent, bool disappearWhenHide = false)
        {
            if (_user == null) return null;
        ...
        /// <param name="afterDestroy">Destroy 후 실행시킬 함수</param>
        public ParticleDestroyer SpawnAndDestroyAfterDuration(string address, Vector2 position,
            UnityAction afterDestroy, bool setEffectParent, bool disappearWhenHide = false)
        {
            if (_user == null) return null;
        ...
        public void Remove(string address)
        {
            ...
            {
                var effects = spawnedEffects[address].ToList();
                spawnedEffects[address].Clear();
                allEffects = allEffects.Except(effects).ToList();
                effects.ForEach(effect => { effect.StopEmitting(); });
            }
        }
        ...
        {
            spawnedEffects.Values.ForEach(x => { x.Remove(effect); });
        ...
        public void RemoveAllEffects()
        {
            spawnedEffects.Values.ForEach(effects =>
            {
                ...
                temp.ForEach(effect => { effect.StopEmitting(); });
            });
            allEffects.Clear();
        }
        ...
        private void ReturnAllEffects()
        {
            ...
            {
                var temp = effects.ToList();
                temp.ForEach(effect => { effect.Return(); });
            });
            allEffects.Clear();
