...
namespace Apis
{
    public class AddressablePooling
    {
        ...
        private void EnQueue(GameObject obj, string address)
        {
            if (obj != null)
            {
                if (!queue.ContainsKey(address)) queue.Add(address, new Queue<GameObject>());
        ...
        private void CreateNew(string address)
        {
            var obj = ResourceUtil.Instantiate(address, pool.transform);
        ...
        public GameObject Get(string address, Vector2? pos = null)
        {
            ...
            if (queue[address].Count == 0) CreateNew(address);
            ...
            if (obj != null)
            {
                ...
                foreach (var temp in list) temp.OnGet();
            }
        ...
        public void Return(GameObject obj)
        {
            ...
            if (_objectInfos.TryGetValue(obj, out var info))
            {
                ...
                {
                    var list = obj.GetComponents<IPoolObject>();
                    foreach (var temp in list) temp.OnReturn();
                }
