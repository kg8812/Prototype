...
namespace Default
{
    public class SoundManager
    {
        ...
        private readonly Dictionary<string, CustomQueue<AudioSource>> playingSFXSources = new();

        private readonly Queue<string> reservations = new();
        private readonly List<AudioSource> SceneBGMSources = new();
        private AudioMixer _audioMixer;

        private TA_SceneMusic.SceneBGMInfo _sceneBGMInfo;
        private AudioSourceUtil ArenaBGMSource;
        private readonly Dictionary<AudioSource, Sequence> AudioSequences = new();


        private int currentIndex;
        private readonly Dictionary<AudioSource, Tween> fadeTweens = new();
        public bool[] isMute = new bool[(int)Define.Sound.MaxCOUNT];

        private bool isReserved;
        private string lastPath = "";
        private Dictionary<string, AudioClip> loadedClips = new();
        private readonly Dictionary<AudioSource, bool> pauseCheckDict = new();
        ...
        public void Init()
        {
            if (_audioMixer == null) _audioMixer = ResourceUtil.Load<AudioMixer>(Define.SoundMixer.AudioMixer);
            ...
            for (var i = 0; i < _baseAudioSource.Length; i++)
            {
                _baseAudioSource[i].dopplerLevel = 0;
                _baseAudioSource[i].reverbZoneMix = 0;
            }

            _baseAudioSource[(int)Define.Sound.BGM].outputAudioMixerGroup =
                ResourceUtil.Load<AudioMixerGroup>(Define.SoundMixer.BGMMixer);
            _baseAudioSource[(int)Define.Sound.SFX].outputAudioMixerGroup =
                ResourceUtil.Load<AudioMixerGroup>(Define.SoundMixer.SFXMixer);
            _baseAudioSource[(int)Define.Sound.UI].outputAudioMixerGroup =
                ResourceUtil.Load<AudioMixerGroup>(Define.SoundMixer.UIMixer);
            _baseAudioSource[(int)Define.Sound.Ambience].outputAudioMixerGroup =
                ResourceUtil.Load<AudioMixerGroup>(Define.SoundMixer.AmbienceMixer);
            _baseAudioSource[(int)Define.Sound.Master].outputAudioMixerGroup =
                ResourceUtil.Load<AudioMixerGroup>(Define.SoundMixer.MasterMixer);
            ...
            LoadAll(Define.Labels.SFX); // sfx 전부 로드
            LoadAll(Define.Labels.BGM); // bgm 전부 로드
        ...
        private Tween DoFadeIn(AudioSource source, float endValue, float fadeTime)
        {
            if (fadeTweens.TryGetValue(source, out var tween)) tween?.Kill();
        ...
        private Tween DoFadeOut(AudioSource source, float fadeTime)
        {
            if (fadeTweens.TryGetValue(source, out var tween)) tween?.Kill();
        ...
        private Sequence GetSequence(AudioSource source)
        {
        ...
        private void KillSequence(AudioSource source)
        {
        ...
        private AudioSource CreateAudioSource(string sourceName, Define.Sound soundType)
        {
        ...
        private void UnPause(AudioSource source)
        {
        ...
        private bool IsPaused(AudioSource source)
        {
            if (pauseCheckDict.TryGetValue(source, out var isPause)) return isPause;

            return false;
        }

        private void LoadAll(string label) // 테스트용
        {
            var list = ResourceUtil.LoadAll<AudioClip>(label);
            list.ForEach(y => { loadedClips.TryAdd(y.name, y); });
        }
        ...
        public void Play(string path, Define.Sound type = Define.Sound.SFX, bool change = true)
        {
            ...
            if (path.Contains("Sounds/" + type + "/") == false) path = $"Sounds/{type}/{path}";
        ...
        public void StopSceneBGM()
        {
            ...
            {
                if (x.isPlaying) DoFadeOut(x, 1).onComplete += x.Stop;
            });
        }
        ...
        public void PlaySceneBGM(TA_SceneMusic.SceneBGMInfo info)
        {
            ...
            var count = SceneBGMSources.Count;
            for (var i = count; i < info.clipAddresses.Count; i++)
            {
            ...
            for (var i = info.clipAddresses.Count; i < count; i++)
            {
                SceneBGMSources[i].clip = null;
                SceneBGMSources[i].Stop();
            }

            for (var i = 0; i < info.clipAddresses.Count; i++)
            {
                var path = info.clipAddresses[i];
                if (path.Contains("Sounds/BGM/") == false) path = $"Sounds/BGM/{path}";
                var temp = i;
                var seq = GetSequence(SceneBGMSources[i]).SetUpdate(true);
                ...
                if (SceneBGMSources[i].isPlaying)
                {
                    ...
                    seq.AppendCallback(() => { SceneBGMSources[temp].Play(); });
                }
                else
                {
                ...
                seq.onKill += () => { SceneBGMSources[info.initialNumber].volume = volume[(int)Define.Sound.BGM]; };
            }
        ...
        public void PauseSceneBGM(float fadeTime)
        {
            ...
            {
                KillSequence(x);
                DoFadeOut(x, fadeTime).onComplete += () => { Pause(x); };
            });
        }
        ...
        public void PlayArenaBGM(List<string> address, float fadeTime)
        {
            ...
            {
                var x = address[i];
                if (x.Contains("Sounds/BGM/") == false) x = $"Sounds/BGM/{x}";
        ...
        public void StopArenaBGM(float fadeTime)
        {
            ...
            KillSequence(ArenaBGMSource.AudioSource);
            DoFadeOut(ArenaBGMSource.AudioSource, fadeTime).onComplete += () => { ArenaBGMSource.Stop(); };
        ...
        public AudioSourceUtil PlayInPosition(string clipAddress, string audioSettingAddress, Vector2 playPos,
            Define.Sound sound)
        {
            if (string.IsNullOrEmpty(clipAddress)) return null;

            playingSFXSources.TryAdd(clipAddress, new CustomQueue<AudioSource>());
            ...
            source.clip = clip;
            var util = Utils.GetOrAddComponent<AudioSourceUtil>(source.gameObject);
        ...
        public AudioSourceUtil PlayInPosition(string[] clipAddresses, string audioSettingAddress,
            AudioSourceUtil.PlayingType playingType, Vector2 playPos, bool isLoop, Define.Sound soundType)
        {
            if (clipAddresses == null || clipAddresses.Length == 0) return null;

            var util = GameManager.Factory.Get<AudioSourceUtil>(FactoryManager.FactoryType.Normal, audioSettingAddress,
                playPos);
            var source = util.AudioSource;
            source.outputAudioMixerGroup = _baseAudioSource[(int)soundType].outputAudioMixerGroup;
        ...
        private IEnumerator PlayNextBGM(AudioSource audio, string path)
        {
        ...
        public void FadeVolume(float amount, float fadeTime, Define.Sound soundType)
        {
            ...
            volumeFade?.Kill();
            volumeFade = _audioMixer.DOSetFloat(soundType.ToString(), Mathf.Log10(amount) * 20, fadeTime)
                .SetAutoKill(false);
            volumeFade.onKill += () =>
            {
        ...
        public void ChangeVolume(float volume, Define.Sound type = Define.Sound.SFX)
        {
            ...
            if (!isMute[(int)type]) _audioMixer.SetFloat(type.ToString(), Mathf.Log10(volume) * 20);
        ...
        {
            var audioSource = _baseAudioSource[(int)type];
            lastPath = "";
