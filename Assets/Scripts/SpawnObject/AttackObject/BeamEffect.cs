...
namespace Apis
{
    public class BeamEffect : Projectile
    {
        ...
        [TabGroup("빔 설정")] [LabelText("발사 시간")]
        public float fireTime;

        [TabGroup("빔 설정")] [LabelText("사정거리")] public float distance;

        [TabGroup("빔 설정")] [LabelText("크기")] public float size;
        ...
        protected override void FixedUpdate()
        {
            ...
            if (isFire) transform.localPosition = pos;
        }
        ...
        public override void Fire(bool rotateWithPlayerX = true)
        {
            ...
            switch (fireDir)
            {
                case FireDir.Horizontal:
                    ...
                    child.DOLocalMoveX(distance / 2f * (_direction != null ? (int)_direction.Direction : 1), fireTime)
                        .SetEase(ease);
                    child.DOScaleX(distance * (_direction != null ? (int)_direction.Direction : 1), fireTime)
                        .SetEase(ease);
                    break;
        ...
        public override void LookAtTarget(IOnHit target, bool setTarget = true)
        {
            ...
            if (target.Position.x < transform.position.x) angle += 180;
