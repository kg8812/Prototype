...
namespace Apis
{
    ...
    public class Projectile : AttackObject, IDirection
    {
        ...
        public Action<IOnHit> OnSetTarget;
        ...
        protected virtual void Update()
        {
            ...
            {
                var scale = ThisTrans.localScale;
        ...
        protected virtual void FixedUpdate()
        {
            ...
            if (IsCheckMaxDist && !Mathf.Approximately(maxDistance, 0))
            {
                ...
                var distanceThisFrame = Vector3.Distance(perPos, position);
        ...
        public override void OnTriggerEnter2D(Collider2D col)
        {
            ...
            else if (col.gameObject.layer == Layers.Ground) OnObjectConflicted(groundConflictType, LayerMasks.Ground);
        }
        ...
        public override void Init(ProjectileInfo info)
        {
            ...
            extensions?.ForEach(x => { x.Init(info); });
        }
        ...
        {
            var normalVel = rigid.linearVelocity.normalized;
        ...
        private void SetVelocityToActorDirection()
        {
            rigid.linearVelocity =
                new Vector2(Mathf.Abs(rigid.linearVelocity.x) * (_direction != null ? (int)_direction.Direction : 1),
        ...
        public void RotateBeforeFire(float angle)
        {
            ...
            var rad = transform.rotation.eulerAngles.z * Mathf.Deg2Rad;
        ...
        public virtual void Fire(bool rotateWithPlayerX = true)
        {
            ...
            if (rotateWithPlayerX) SetVelocityToActorDirection();
        ...
        protected virtual void OnObjectConflicted(ProjectileConflictType conflictType, LayerMask layerMasks)
        {
            switch (conflictType)
            {
                ...
                case ProjectileConflictType.Reflect:
                    var hit = Physics2D.Raycast(transform.position, rigid.linearVelocity, reflectionCheckDist,
                        layerMasks);
                    if (hit.collider != null)
                    {
                        var normal = hit.normal;
                        var dotProduct = Vector2.Dot(rigid.linearVelocity, normal);
                        rigid.linearVelocity -= 2f * dotProduct * normal;
                    }

                    break;
                case ProjectileConflictType.Penetrate:
                    if (penetrationCount >= penetrationMax) Destroy();
                    penetrationCount++;
        ...
        public override void Destroy()
        {
            ...
            isDestroyed = true;
            extensions.ForEach(x => { x?.Destroy(); });
        }
        ...
        [TabGroup("InfoGroup/공격설정", "투사체 설정")] [LabelText("보스 충돌타입 사용여부")]
        ...
        [TabGroup("InfoGroup/공격설정", "투사체 설정")]
        [LabelText("관통 최대 횟수")]
