...
namespace Apis.CommonMonster2
{
    public partial class CommonMonster2
    {
        ...
        {
            if (!ReferenceEquals(_playerTrans, null)) return _playerTrans.position.x > transform.position.x;
            return false;
        ...
        {
            var playerRL = IsPlayerRight() ? EActorDirection.Right : EActorDirection.Left;
        ...
        {
            var myRayPoint = Position;
            Vector2 dir = GameManager.instance.ControllingEntity.Position - myRayPoint;
            var hit = Physics2D.Raycast(myRayPoint, dir, Mathf.Infinity, LayerMasks.Player | LayerMasks.GroundWall);
#if UNITY_EDITOR
