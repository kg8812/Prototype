...
namespace Default
{
    public class UI_Scene : UI_Base, IController
    {
        ...
        protected override void Activated()
        {
            ...
            if (!isOverlay) _pauseGuid = GameManager.instance.RegisterPause();

            if (dontShowMainCam) CameraManager.instance.ToggleMainCamCullingMask(false);
        }

        protected override void Deactivated()
        {
            base.Deactivated();
            if (!isOverlay) GameManager.instance.RemovePause(_pauseGuid);
        }

        public override void TryDeactivated(bool force = false)
        {
            if (dontShowMainCam) CameraManager.instance.ToggleMainCamCullingMask(true);
