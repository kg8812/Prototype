...
public class ConveyorBelt : MonoBehaviour
{
    ...
    [LabelText("이동속도 (m/s)")] [SerializeField]
    private float speed;
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        ...
        {
            var target = other.transform.parent != null ? other.transform.parent : other.transform;

            if (!moves.Contains(target)) moves.Add(target);
        }
    }
    ...
    private void OnTriggerExit2D(Collider2D other)
    {
        if (((1 << other.gameObject.layer) & layers) != 0)
        {
            ...
            if (moves.Contains(target)) moves.Remove(target);
        }
    }
