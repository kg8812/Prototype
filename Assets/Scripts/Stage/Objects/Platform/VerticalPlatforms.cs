...
{
    [LabelText("이동방향")] [SerializeField] private Direction direction = Direction.Up;
    [LabelText("이동시간")] [SerializeField] private float moveTime;
    [LabelText("이동Ease")] [SerializeField] private Ease moveEase;

    [InfoBox("입력하면 씬창에서 해당위치에 빨간색(시작점),파란색(끝점)이 표시됩니다.")] [LabelText("시작점")] [SerializeField]
    private float startPoint;

    [LabelText("끝점")] [SerializeField] private float endPoint;
    [LabelText("생성시간")] [SerializeField] private float creationTime;
    ...
    {
        var platform = base.CreatePlatform(position);
    ...
    public override void Return(GameObject platform)
    {
        ...
        if (platform.TryGetComponent(out SpriteRenderer render))
        {
            ...
            tween.onComplete += () => { render.color = color; };
        }
        else
        {
            Func();
    ...
    private void Create()
    {
        var platform = CreatePlatform(new Vector2(transform.position.x,
            transform.position.y + (direction == Direction.Down ? startPoint : endPoint)));
        curTime = creationTime;
        var tween = platform.transform
            .DOMoveY(transform.position.y + (direction == Direction.Down ? endPoint : startPoint), moveTime)
            .SetEase(moveEase);
        tween.onComplete += () => { Return(platform); };
    }
