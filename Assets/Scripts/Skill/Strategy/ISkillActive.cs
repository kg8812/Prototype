...
namespace Apis
{
    ...
    public class ChargeSkill : ISkillActive // 차지형
    {
        ...
        {
            ResourceUtil.Load<ActorCommand>(Define.ActorCommands.EndWeaponSkill)
        };

        private static List<ActorCommand> activeSkillEndCommand => _activeSkillEndCommand ??= new List<ActorCommand>
        {
            ResourceUtil.Load<ActorCommand>(Define.ActorCommands.EndActiveSkill)
        };
        ...
        public void Activate(ActiveSkill skill)
        {
            ...
            seq.onUpdate += () =>
            {
                skill.CurChargeTime += Time.deltaTime;
